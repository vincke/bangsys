$(function() {
	...
	$('[name="search_header"]').on('keydown', function (e) {
        ...
        if (key == 13 ) {
            e.preventDefault();
            search_form.action = eho.BASE_PATH + "front/tags/"+ $('[name="search_header"]').val();
    		search_form.submit();
        }
    });
...
comment = {
	...
	oper_del:function(comment_id,article_id){
		if(window.confirm('你确定要删除该评论吗？')){
			jQuery.ajax({
				type:'POST',
				url:eho.BASE_PATH + 'front/comment/del',
	...
	,oper_save:function(comment_content,article_id,reply_userid,create_id , article_title){
		...
		jQuery.ajax({
			type:'POST',
			url:eho.BASE_PATH + 'front/comment/save',
			data:urlParams,
			success:function(data){
			if(data.status==1){
				...
				title_url = (title_url=='')?(eho.BASE_PATH + 'static/images/user/user.png'):title_url;
	...
	,count:function(){
		...
		jQuery.ajax({
			type:'POST',
			url:eho.BASE_PATH + 'front/comment/count',
			success:function(data){
				if(data.status==1){
					if(data.count > 0 ){
						$('#mymessage').hide();
...
articlelike = {
	...
	,yes:function(article_id){
		jQuery.ajax({
			type:'POST',
			url:eho.BASE_PATH + 'front/articlelike/yes/'+article_id,
			success:function(data){
				if(data.status==1){
	...
	,no:function(article_id){
		jQuery.ajax({
			type:'POST',
			url:eho.BASE_PATH + 'front/articlelike/no/'+article_id,
			success:function(data){
				if(data.status==1){
